...
The user wants component recommendations. Based on what they describe, identify the most relevant RAD components and explain:
1. Which components to use and why
2. What behavioral contracts apply
3. What to avoid

If no description was provided above, ask: "What are you building? Describe the AI behavior, actions, or outputs your interface needs to handle."
...
**R — Responsible.** Every AI action is attributable. Systems surface what they did, on what basis, and under whose authority.

**A — Accountable.** Human oversight is preserved. Consequential decisions require explicit human approval before execution. Never auto-approve; auto-cancellation only.

**D — Disclosed.** AI involvement is never hidden. Systems identify themselves, their data sources, and their confidence levels.

---

## Key Design Rules

- **Never auto-approve** an agent action on timer expiry. Auto-cancellation only.
- **Consequential decisions require explicit human approval.** Implicit consent from prior sessions is not acceptable.
- **Every irreversible agent action requires a pre-action warning.** Every detected error requires a recovery surface.
- **Enhancement is always opt-in.** Never silently rewrite a user's prompt.
- **The result is always primary.** The process view is always available. Defaulting to the process view is an error.
- **Suggested prompts must reflect what the interface can actually do.** Do not suggest actions the system cannot perform.
